  ...
  return (
    <div className='py-3'>
      <div className='flex items-center justify-center pb-2'>
        <div>
          <select
            className='h-[40px] w-[150px] cursor-pointer flex-wrap border py-2 text-center outline-none transition-all duration-300 hover:border-theme-btn-secondary'
            value={selectInput}
        ...
        <button
          ...
          className='root-btn h-[40px] bg-theme-btn-secondary text-white'
        >
          Save
        </button>
      </div>
          ...
          {grid.map((item, rowIdx) => (
            <React.Fragment key={rowIdx}>
              {item.map((cell, colIdx) => {
                ...
                const isCurrentItem =
                  rowIdx === currentItem.row && colIdx === currentItem.col;
